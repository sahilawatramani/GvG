...
Use this section if you want to connect a frontend to the API without changing the training or preprocessing code.

### Backend files your frontend will use

- `app.py` - FastAPI application with the HTTP endpoints
- `training/config.py` - central path configuration for model and artifact locations
- `training/ids_model.py` - loads the trained model and produces predictions
- `custom_input/runner.py` - aligns incoming columns to the expected feature order
- `artifacts/feature_names.txt` - required feature list used by the API for alignment
- `artifacts/models/robust_ids.pt` - trained model file required before prediction
...
If your frontend is hosted elsewhere, the API already allows browser requests through CORS.
...
```bash
docker build -t gvg-api .
```

Run the container:

```bash
docker run --rm -p 8000:8000 gvg-api
```
...
If you want to keep model/data artifacts outside the image, mount them at runtime:
...
- `GET /` - health check
- `POST /predict` - returns predictions for one row or multiple rows

### Request format

Send JSON, either as a single object or as an array of objects.

Example single-row payload:
...
You do not need to send every column manually if your frontend is only passing a subset. Missing columns are filled with `0.0` by the API using the same alignment logic as the custom-input runner.

### Response format

The API returns JSON shaped like this:
...
If enough rows are provided for sequence scoring, `sequence_predictions` will contain a list of window-level results.
...
1. Make sure the backend dependencies are installed from `requirements.txt`.
2. Start the API with `python app.py` from the project root.
3. Call `POST /predict` from the frontend and send JSON traffic features.
4. Read the prediction from `tabular_predictions[0].predicted_binary_label` and `attack_probability`.
5. If sequence output is needed, read `sequence_predictions` when it is not `null`.

### Important runtime files

Before the frontend can query the API, these files must exist:

- `artifacts/models/robust_ids.pt`
- `artifacts/feature_names.txt`
- `preprocessed/splits/X_test.csv`

Those files are created after preprocessing and training complete successfully.
